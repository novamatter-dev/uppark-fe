export const fr = `CONDITIONS GÉNÉRALES D&#39;UTILISATION DU SITE WEB ET DE L&#39;APPLICATION UPPARK
Les présentes conditions générales constituent l&#39;accord contractuel entre FAST PARK CO S.R.L. et vous,
régissant à la fois les conditions d&#39;accès aux services de FAST PARK et les droits et obligations de
chaque partie.
Le soussigné, FAST PARK CO S.R.L., est une société roumaine, dont le siège est situé à Voluntari, Bd.
Pipera n° 1B, L’Immeuble de bureaux Cubic Center, et. 7, département d&#39;Ilfov, Roumanie et détient les
droits de propriété intellectuelle sur le site web et l&#39;application UPPARK.
Si vous choisissez de télécharger l&#39;application UPPARK et de vous inscrire dans cette application, et
donc d&#39;utiliser les services UPPARK, vous êtes réputé accepter inconditionnellement les présentes
conditions.
Veuillez donc les lire attentivement avant de procéder au téléchargement et à l&#39;enregistrement de vos
données personnelles dans l&#39;application.
Toute responsabilité concernant le contrôle, l&#39;activité et les conséquences de l&#39;accès des mineurs aux
espaces virtuels, relève de la responsabilité des parents ou du tuteur légal du mineur, FAST PARK
COS.R.L. étant exonéré de toute responsabilité.
Nous essayons constamment d&#39;améliorer nos services et, par conséquent, les présentes conditions
générales sont susceptibles d&#39;être modifiées, ce qui sera indiqué sur le site web et via l&#39;application. Si
vous continuez à utiliser nos services, il est présumé que vous acceptez les modifications apportées à
ces conditions générales. Si vous n&#39;êtes pas d&#39;accord avec les changements, vous avez la possibilité de
cesser d&#39;utiliser les services de FAST PARK. Ces conditions générales ont été mises à jour le
……………, date à laquelle elles n&#39;ont pas subi d&#39;autres modifications.
UPPARK vous propose un mode de paiement à distance, à partir de votre téléphone mobile ou d&#39;un autre
équipement mobile compatible (tablette, ordinateur portable, etc.), pour les services de stationnement
proposés par nos partenaires. Vous pouvez payer ces services soit en utilisant votre carte bancaire, soit
via votre opérateur mobile, auquel cas vous optez pour le paiement par SMS.
Veuillez noter que FAST PARK n&#39;offre pas de services de stationnement et que les présentes
conditions générales ne constituent pas un contrat de stationnement. FAST PARK offre
uniquement des services pour faciliter le paiement des frais de stationnement aux opérateurs
publics ou privés. Le contrat pour les services de stationnement est conclu entre vous et le fournisseur
de services de stationnement dans la zone/ville où vous stationnez.
Les services UPPARK ne sont disponibles que dans certaines régions de Roumanie et nous ne
garantissons pas leur disponibilité dans tout le pays.

Les termes suivants ont la signification indiquée ci-dessous:
Compte(s) désigne(nt) l&#39;ensemble des comptes de service de paiement de stationnement que vous avez
ouverts sur l&#39;application UPPARK;
Service désigne les services offerts par FAST PARK CO S.R.L. pour faciliter le paiement des services
des prestataires partenaires pour lesquels vous optez via l&#39;application (par exemple, paiement de tarifs
horaires ou de cartes de stationnement, paiement de tickets ou de cartes de transport public, etc.) en
utilisant votre carte bancaire, votre téléphone ou tout autre équipement mobile compatible, conformément
aux présentes conditions générales;

FAST PARK est la société FAST PARK COS.R.L., dont le siège est situé à Voluntari, Bd. Pipera n° 1B,
L’Immeuble de bureaux Cubic Center, et.7, Ilfov County, Roumanie, ainsi que nos successeurs, affiliés
ou associés et toute autre entité que le soussigné ou nos associés pourraient créer à l&#39;avenir;
Le site web, ou le site, désigne le site web actuellement utilisé ainsi que tout successeur de ce site web,
sur lequel FAST PARK détient les droits de propriété intellectuelle;
UPPARK désigne l&#39;application de paiement du stationnement par carte bancaire ou SMS, dont FAST
PARK détient les droits de propriété intellectuelle.

UTILISATION ET PAIEMENT DES SERVICES
Pour accéder aux services de facilitation du stationnement FAST PARK, vous devez télécharger
l&#39;application UPPARK, disponible dans l&#39;AppStore et le GooglePlay Store, sur votre téléphone portable.
Après le téléchargement, pour vous connecter à l&#39;application, vous devez soit créer un compte
d&#39;utilisateur avec le mot de passe de votre choix, en fournissant les données personnelles demandées,
soit utiliser un compte Facebook ou Google valide. Pour la politique de traitement des données
personnelles, veuillez vous référer à la section ci-dessous.
Assurez-vous que la zone que vous sélectionnez est la même que celle qui figure sur les
panneaux d&#39;information de l&#39;endroit où vous vous êtes garé.
Le client est responsable de l&#39;indication du numéro de stationnement et d&#39;immatriculation correct.
Le service de localisation de l&#39;application sert à indiquer les parkings les plus proches.
Pour les paiements de services par carte bancaire:
Pour activer votre compte, vous devez entrer les détails d&#39;une carte bancaire valide qui vous appartient et
votre numéro de téléphone pour l&#39;option de paiement par SMS.
Lorsque vous activez votre compte, l&#39;application génère un jeton de sécurité, qui est un code informatique
unique permettant d&#39;identifier votre compte et les paiements que vous effectuez.
Afin de générer le jeton, nous devons vous facturer un paiement temporaire de 1 leu, qui vous sera
remboursé après l&#39;achèvement de la procédure d&#39;activation du compte, dans un délai raisonnable, en
fonction de la politique de la banque émettrice de votre carte.
Pour activer votre compte, vous devez également enregistrer le numéro d&#39;immatriculation de votre
véhicule.
Après avoir activé votre compte, vous pourrez utiliser nos services de facilitation du paiement du
stationnement, ce qui signifie que vous devez identifier la zone dans laquelle vous vous trouvez pour
payer le tarif correct applicable à l&#39;endroit où vous vous êtes garé.
Vous sélectionnerez la durée de stationnement estimée dans l&#39;application.
Vous pouvez choisir le mode de paiement: par carte bancaire ou par SMS.
Si vous choisissez de payer par carte, veuillez noter que les paiements seront traités par Netopia
Payments. Si une demande de paiement est refusée par votre banque, vous devez payer le service de
stationnement au moyen d&#39;une autre carte bancaire ou d&#39;un SMS.
Après avoir effectué un paiement par carte bancaire, l&#39;application génère automatiquement la facture
correspondante, qui est envoyée à l&#39;adresse électronique enregistrée dans votre compte d&#39;utilisateur.

FAST PARK ne demande pas et ne conserve pas les données de votre carte, autres que celles requises
exclusivement pour la plateforme de paiement en ligne.
FAST PARK accepte les cartes: Visa, Mastercard, Discover, AmericanExpress, JCB, WrightExpress,
Carte Blanche et Diner&#39;s Club.
Par le biais de l&#39;application UPPARK, les paiements ne peuvent être effectués qu&#39;à FAST PARK ou à nos
partenaires, fournisseurs de services de stationnement.
Pour les paiements de services par SMS:
Le paiement par SMS sera effectué conformément aux instructions figurant dans l&#39;application. Le numéro,
le texte du SMS et le code sont différents selon la ville. Par conséquent, veuillez vérifier l&#39;exactitude de
ces données avant d&#39;effectuer la transaction.
Lorsque la durée estimée de la période de stationnement est terminée, vous recevez une notification et
vous avez la possibilité de prolonger cette période. Le stationnement peut être prolongé manuellement
par l&#39;utilisateur en payant le tarif approprié
Dans certaines localités, nous vous offrons également la possibilité d&#39;acheter des abonnements de
parking, des tickets et des abonnements de transport public via l&#39;application, dans les mêmes conditions
de paiement.
En cas de paiement par SMS, le montant de la transaction apparaîtra sur la facture de l&#39;opérateur mobile
et vous recevrez également un SMS confirmant l&#39;enregistrement du paiement.
Si la facture n&#39;est pas chargée sur votre compte ou si vous ne recevez pas de SMS de confirmation dans
les deux minutes qui suivent le paiement, cela signifie que la transaction a échoué, auquel cas vous êtes
considéré comme n&#39;ayant PAS payé le service de stationnement et vous n&#39;avez donc pas le droit de
stationner sur la place de parking en question.
Il vous revient de vérifier le traitement de la transaction. Nous ne prenons aucune responsabilité dans le
cas où le paiement n&#39;a pas été traité et que vous êtes par conséquent sanctionné par le fournisseur de
services de stationnement.
Nous réitérons le fait que nous ne fournissons pas de services de stationnement, mais uniquement une
facilitation de paiement. Nous ne sommes donc pas responsables de la sécurité et de l&#39;intégrité de votre
véhicule ou des biens qui s&#39;y trouvent.
IMPORTANT
Si l&#39;option de réception des notifications UPPARK est décochée, que ce soit dans l&#39;application ou dans
les paramètres généraux du téléphone, l&#39;utilisateur ne sera pas informé de l&#39;expiration imminente d&#39;une
réservation de produit dans l&#39;application. Nous vous recommandons d&#39;activer toutes les notifications pour
UPPARK.
Pour activer les notifications du système, pour iOS, allez dans le menu Paramètres - Notifications, puis
sélectionnez l&#39;appli UPPARK et cochez Autoriser les notifications.
Pour Android, allez dans le menu Paramètres - Notifications, puis sélectionnez l&#39;application UPPARK et
cochez l&#39;option Afficher les notifications.

LA DISPONIBILITÉ DU SERVICE
Le service UPPARK est disponible dans certaines régions de Roumanie et nous ne garantissons pas sa
disponibilité dans tout le pays, bien que nous essayions constamment de l&#39;étendre.

Les tarifs de stationnement sont constamment mis à jour et peuvent varier en fonction de l&#39;emplacement,
du zonage et de l&#39;opérateur de stationnement.
Nous vous informons que nous ne prenons aucune responsabilité, ni envers vous ni envers des tiers,
pour la modification, la suspension ou l&#39;interruption des services.
Nous n&#39;avons aucune obligation de fournir les services de manière ininterrompue ou sans erreur et ne
serons en aucun cas responsables envers vous des conséquences d&#39;éventuelles interruptions ou erreurs.
Vous acceptez le fait que des interruptions peuvent se produire pour des raisons techniques, des
limitations technologiques (signal gsm/LTE) ainsi que la technologie de traitement des paiements
électroniques ou d&#39;autres raisons qui ne sont pas la faute de FAST PARK, que nous ne pouvons pas
contrôler et qui peuvent affecter la disponibilité ou la qualité des services.
Cependant, nous assumons l&#39;obligation de rétablir tous les services affectés dès que possible.
Vous êtes responsable de l&#39;obtention de l&#39;accès au réseau internet nécessaire à l&#39;utilisation des services
fournis et du paiement de tous les frais et redevances appliqués par votre fournisseur de réseau mobile, y
compris l&#39;utilisation des données lors de l&#39;utilisation des applications UPPARK. Vous êtes également
responsable de l&#39;accès aux appareils compatibles et aux systèmes d&#39;exploitation requis pour utiliser les
services internet, y compris toute mise à jour de logiciel ou de matériel.
L&#39;utilisateur déclare être le propriétaire légitime des moyens de paiement utilisés pour le paiement. FAST
PARK n&#39;est pas responsable des réclamations liées à l&#39;utilisation incorrecte des moyens de paiement par
d&#39;autres personnes que le titulaire et ne peut pas rembourser le montant payé si l&#39;utilisateur a effectué
une transaction erronée ou non autorisée par le titulaire.

L&#39;UTILISATION DE TÉLÉPHONES OU D&#39;AUTRES APPAREILS MOBILES PENDANT LA CONDUITE
L&#39;utilisateur est informé qu&#39;il est dangereux d&#39;utiliser des téléphones portables ou d&#39;autres appareils
mobiles en conduisant. FAST PARK conseille à l&#39;utilisateur de ne pas utiliser le service UPPARK lorsqu&#39;il
manipule un véhicule en mouvement. L&#39;utilisateur s&#39;engage à dégager FAST PARK de toute
responsabilité en cas de dommages résultant de l&#39;utilisation du service UPPARK lors de la manipulation
de tout type de véhicule.
L&#39;utilisateur est responsable des amendes de stationnement reçues (ou des avis de non-paiement du
stationnement ou des avis de paiement incorrect) et de leur paiement. FAST PARK n&#39;a aucune
compétence entre l&#39;utilisateur du parking et le gestionnaire du parking (municipalité, autorités locales ou
entités privées).

LA POLITIQUE DE SÉCURITÉ DES DONNÉES
Les données que vous nous fournissez sont stockées sur un serveur appartenant à FAST PARK, pour
lequel nous avons pris toutes les mesures de sécurité physiques et informatiques.
Nos systèmes sont rigoureusement testés et ont été développés avec des logiciels de cryptage et des
technologies sécurisées. Toutes les données bancaires et les transactions sont également protégées par
des pare-feux afin de garantir la sécurité de toutes les données personnelles. L&#39;utilisation des données de
votre carte bancaire est conforme aux normes de sécurité.
En tant qu&#39;utilisateur enregistré, vos données ne seront disponibles qu&#39;aux fins approuvées des services
et aucune autre utilisation n&#39;est possible, à l&#39;exception des paiements à l&#39;abonné ou à nos fournisseurs
avec lesquels nous avons une relation de travail. Même dans le cas improbable d&#39;un accès non autorisé

à vos données, notre système ne vous permet pas d&#39;effectuer d&#39;autres paiements que ceux mentionnés
ci-dessus.
Vous pouvez consulter et modifier les données enregistrées via l&#39;application UPPARK.
Si vous avez des raisons de suspecter une utilisation non autorisée de ces données, y compris les détails
de la carte bancaire, vous devez nous en informer immédiatement en nous contactant via le formulaire
sur le site web. Si vous avez déclaré que votre carte a été volée, les mesures de sécurité habituelles
prévues par les lois applicables seront appliquées et votre compte sera suspendu afin d&#39;empêcher toute
utilisation non autorisée. Pour pouvoir utiliser à nouveau les services de FAST PARK, vous devrez
prendre les mesures nécessaires pour modifier vos données de paiement.
Si vous pensez qu&#39;il existe un risque de sécurité pour votre compte, veuillez nous en informer au moyen
du formulaire figurant sur le site web. Si vous pensez que votre téléphone portable a été volé ou utilisé à
mauvais escient, vous devez nous en informer dès que possible afin de limiter toute utilisation ultérieure
du téléphone pour les services FAST PARK et de limiter les implications financières que vous pourriez
avoir à la suite du vol. Si vous ne nous informez pas en temps utile d&#39;un tel événement, nous n&#39;assumons
aucune responsabilité pour les transactions frauduleuses effectuées par des tiers dans de telles
circonstances.
Vous vous engagez à ne pas prendre de mesures pour abuser du service UPPARK, y compris, sans s&#39;y
limiter, la tentative d&#39;obtenir un accès non autorisé au site ou à toute partie des systèmes FAST PARK ou
d&#39;introduire sciemment des virus, des vers ou d&#39;autres logiciels malveillants.
Nous nous réservons le droit de refuser votre inscription et/ou de suspendre et/ou de résilier votre
compte si nous estimons que vous avez enfreint ou risquez d&#39;enfreindre les présentes conditions
générales ou si la loi l&#39;exige, et nous ne serons pas tenus de vous fournir les raisons de cette décision.
Nous pouvons également suspendre votre compte si nous soupçonnons une utilisation non autorisée ou
un abus.
Nous utilisons des cookies pour vous montrer les informations et les paramètres corrects lorsque vous
utilisez l&#39;application UPPARK.
Nous utilisons des cookies de sécurité pour l&#39;authentification de l&#39;utilisateur, afin d&#39;empêcher l&#39;utilisation
frauduleuse de la connexion et de protéger les données de l&#39;utilisateur contre les parties non autorisées.

LES DROITS DE PROPRIÉTÉ INTELLECTUELLE
L&#39;application UPPARK, les services et le site web sont protégés par des droits de propriété intellectuelle
appartenant à FAST PARK CO S.R.L. ou à ses filiales. Ces droits de propriété intellectuelle s&#39;appliquent
également aux produits dérivés, aux nouvelles versions, aux améliorations, aux mises à jour et aux
modifications.
La marque UPPARK et toutes les autres marques figurant sur le site web de FAST PARK sont des
marques déposées de FAST PARK CO S.R.L. ou de ses filiales.
La marque UPPARK ne peut être utilisée en relation avec un produit ou un service qui n&#39;appartient pas à
FAST PARK CO S.R.L. ou à ses sociétés affiliées, d&#39;une manière susceptible de créer une confusion
parmi les clients ou d&#39;une manière qui dénigre FAST PARK CO S.R.L..
En activant votre compte dans l&#39;application UPPARK, vous acquérez de FAST PARK une licence non
exclusive, temporaire et non transférable pour utiliser les produits FAST PARK, sur lesquels nous
détenons des droits de propriété intellectuelle. En dehors de ceux expressément mentionnés dans les
présentes conditions générales, FAST PARK ne vous accorde aucun autre droit de propriété

intellectuelle, et vous êtes tenu de vous abstenir de toute violation des droits de FAST PARK. Dans le cas
contraire, vous serez tenu responsable de tous les dommages de quelque nature que ce soit résultant
d&#39;une violation par vous des droits de propriété intellectuelle de FAST PARK.
Toute communication, y compris, mais sans s&#39;y limiter, les e-mails, les images, les clips audio, les vidéos,
les graphiques et/ou d&#39;autres matériaux envoyés directement ou par copie ou autrement par vous à FAST
PARK ou à l&#39;un de nos employés, avocats ou agents et toute publication sur ce site deviendront la
propriété de FAST PARK dès leur transmission. En soumettant ces informations, vous nous accordez
irrévocablement le droit de les utiliser gratuitement.

DIVERS
La responsabilité de FAST PARK pour tout dommage direct ou indirect que vous pourriez subir suite à
l&#39;utilisation des Services FAST PARK sera limitée au coût du produit acheté ou du
parking/ticket/abonnement.
Les présentes conditions générales constituent un contrat entre vous et FAST PARK et régissent les
services que nous vous fournissons. Si une disposition est ou devient nulle ou inapplicable, elle
n&#39;affectera pas la validité et l&#39;applicabilité des autres dispositions.
En cas de non-respect de ces termes et conditions, FAST PARK se réserve le droit de suspendre ou de
résilier tout compte utilisateur sans préavis. La résiliation du compte en raison du non-respect des
conditions contractuelles ne donne pas droit au remboursement des paiements effectués.

LA LÉGISLATION APPLICABLE ET LES LITIGES
Ces conditions générales sont réglementées par la loi roumaine. Tout litige éventuel entre vous et FAST
PARK sera résolu conformément à la loi roumaine en vigueur à la date du litige et sera soumis aux
tribunaux compétents de notre siège social.
Vous acceptez également de vous conformer à toutes les lois, y compris, mais sans s&#39;y limiter, celles qui
s&#39;appliquent à la conduite en ligne.

J&#39;ai lu l&#39;intégralité de ces conditions et je les accepte - VEUILLEZ COCHER
J&#39;accepte de recevoir régulièrement des lettres d&#39;information m&#39;informant des nouveautés liées
aux services d&#39;UPPARK - VEUILLEZ COCHER
J&#39;accepte de recevoir du matériel promotionnel de FAST PARK CO et de ses filiales - VEUILLEZ
COCHER`;
